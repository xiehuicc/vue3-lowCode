...
const initColumns = () => {
  if (props.component.props.columns) {
    ...
    // 默认列 - 1*2结构（1行2列）
    allColumns.value = [
      { key: 'column1', label: '列1', prop: 'column1', width: '', sortable: true, disabled: false },
      { key: 'column2', label: '列2', prop: 'column2', width: '', sortable: true, disabled: false }
    ];
  }
...
const handleCellContextMenu = (row: any, column: any, cell: HTMLElement, event: MouseEvent) => {
  ...
  // 添加点击其他区域关闭菜单的事件
  document.addEventListener('click', closeCellContextMenu);
};

// 关闭单元格右键菜单
const closeCellContextMenu = () => {
  cellContextMenuVisible.value = false;
  document.removeEventListener('click', closeCellContextMenu);
};
...
const handleDeleteRow = () => {
  if (currentCell.value.row === -1 || tableData.value.length <= 1) return;
...
const handleAddColumn = (position: 'before' | 'after') => {
  ...
  // 创建新列名称
  const newColumnName = `column${allColumns.value.length + 1}`;
  const newColumnLabel = `列${allColumns.value.length + 1}`;
  ...
  // 插入新列配置
  const insertIndex = position === 'before' ? currentColumnIndex : currentColumnIndex + 1;
  allColumns.value.splice(insertIndex, 0, newColumn);
...
const handleDeleteColumn = () => {
  if (!currentCell.value.column || allColumns.value.length <= 1) return;
  ...
  // 删除列配置
  const deletedColumn = allColumns.value.splice(currentColumnIndex, 1)[0];
  
  // 从选中列中移除
  const keyIndex = selectedColumnKeys.value.indexOf(deletedColumn.key);
  if (keyIndex !== -1) {
